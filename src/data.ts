...
export const INITIAL_AD_SLOTS:AdSlot[]=['sponsor-top','slot-1','slot-2','slot-3','slot-4','slot-6'].map((id,i)=>({id,title:['Direct Sponsor — Top of public pages','Header Banner — Top of public pages','Homepage Banner — Under hero section','Jobs Directory Banner — Top of jobs listing','Job Detail Top Banner — Top of job recruitment notice','Sidebar Placement Banner'][i],locationName:['Sponsor Top','Header Banner','Homepage Top','Job Listing Top','Job Detail Top','Sidebar'][i],htmlContent:id==='sponsor-top'?'<div style="padding:18px;text-align:center;background:#fff3cd;border:2px dashed #ff9800;border-radius:10px;font-weight:700;color:#7a4b00">TEST SPONSOR BANNER — Your Advertisement Here</div>':'',enabled:id==='sponsor-top'}));
...
export class JobKhojDataStore {
 ...
 private static async saveRecord(kind:Kind,item:any):Promise<void>{
   ...
   for(const field of urlFields){ if(field in x && x[field]){ const u=new URL(String(x[field]),location.origin); if(u.protocol !== 'https:') throw new Error(`Invalid URL in ${field}: HTTPS is required`); } }
   if(kind==='jobs'){
 ...
 static async saveFeatures(v:FeatureSettings){await this.ensureRemote();if(!this.canWrite())throw new Error('Editor permission required');const clean={...INITIAL_FEATURES,...v,adminUsers:[]};for(const field of ['announcementUrl','popupUrl'] as const){if(clean[field]&&!/^(https?:\/\/|\/|#)/i.test(clean[field]))throw new Error(`Invalid ${field}`);}
if(clean.seoCanonicalUrl){try{const u=new URL(clean.seoCanonicalUrl);if(u.protocol!=='https:'||!u.hostname)throw new Error();}catch{throw new Error('Invalid seoCanonicalUrl: HTTPS absolute URL is required.');}}for(const r of clean.redirectRules){if(!r.from||!r.to||/^javascript:/i.test(r.to)||/^data:/i.test(r.to))throw new Error('Invalid redirect rule');}const {error}=await supabase.from('site_config').upsert({key:'features',value:clean,updated_at:new Date().toISOString()},{onConflict:'key'});if(error)throw error;this.featuresCache=clean}
